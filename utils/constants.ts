...
export const STREAMING_SERVERS = {
  movie: {
    ...
    SuperEmbedVIP: (id: string | number): string => `https://multiembed.mov/directstream.php?video_id=${id}&tmdb=1`,
    EmbedSu: (id: string | number): string => `https://embed.su/embed/movie/${id}`,
    TwoEmbed: (id: string | number): string => `https://www.2embed.stream/embed/movie?id=${id}`,
  ...
  tv: {
    ...
    SuperEmbedVIP: (id: string | number, season: number, episode: number): string => `https://multiembed.mov/directstream.php?video_id=${id}&tmdb=1&s=${season}&e=${episode}`,
    EmbedSu: (id: string | number, season: number, episode: number): string => `https://embed.su/embed/tv/${id}/${season}/${episode}`,
    TwoEmbed: (id: string | number, season: number, episode: number): string => `https://www.2embed.stream/embed/tv?id=${id}&s=${season}&e=${episode}`,
